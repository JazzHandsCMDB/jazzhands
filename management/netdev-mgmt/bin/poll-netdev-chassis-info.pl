...
# Copyright (c) 2023-2025, Todd Kover
# All rights reserved.
#
...
if ($encapdomain) {
	...
	if ($encaptype) {
		die
		  "Setting an encapsulation-type requires setting an encapsulation-domain.\n";
	}
}
...
foreach my $host (@$hostname) {
	...
	if ($filename) {
		...
		if ( !(
			$chassisinfo = $device->GetChassisInfo(
				errors => \@errors,
			)
		) )
		{
	...
	if ( !$db_dev->{device_id} ) {
		...
		if ( !( $ins_dev_sth->execute(
			$host, $chassisinfo->{model}, $site_code
		) ) )
		{
		...
		if ($do_update) {
			if ( !( $upd_dev_sth->execute(
				$db_dev->{device_id}, $host, $chassisinfo->{model}
			) ) )
			{
	...
	if (
		defined( $chassisinfo->{lldp_chassis_id} )
	...
	if ( $chassisinfo->{model} eq 'Juniper EX4xxx virtual chassis' ) {
		...
		foreach
		  my $slot ( sort { $a <=> $b } keys %{ $chassisinfo->{modules} } )
		{
			my $slotname = 'VC' . $slot;
			if ( $components->{$slotname}->{component_id} ) {
				...
				{
					printf STDERR
					  "Serial number of component %s in slot %s (%s) of device %s (%s) do not match.  This needs to be fixed manually because of things\n",
					...
					$chassisinfo->{modules}->{$slot}->{model} )
				{
					if ($verbose) {
						printf
						  "Changing component type of slot %s of %s from %s to %s\n",
						  $slotname,
						  $host,
				...
				if ( !$components->{$slotname}->{serial_number} ) {
					if ($verbose) {
						printf
						  "Setting serial number of %s in slot %s (component id %d) of %s to %s\n",
						  $chassisinfo->{modules}->{$slot}->{model},
						  $slotname,
						  $components->{$slotname}->{component_id},
						  $host,
					...
					if ( $components->{$slotname}->{asset_id} ) {
						...
						{
							printf STDERR
							  "Unable to update serial number: %s\n",
							  $set_sn_sth->errstr;
							exit 1;
						}
					} else {
				...
				if ($verbose) {
					printf
					  "Inserting %s component type into slot %s of %s with serial %s\n",
					  $chassisinfo->{modules}->{$slot}->{model},
					  $slotname,
					  $host,
					  $chassisinfo->{modules}->{$slot}->{serial};
		...
		{
			printf STDERR
			  "Serial number of asset does not match (asset %d: %s vs %s).  This needs to be fixed manually because of things\n",
			  $asset->{asset_id},
			  $asset->{serial_number},
			  $chassisinfo->{serial};
			next;
		...
		if ( !$asset->{component_id} ) {
			if ($verbose) {
				printf
				  "Inserting %s component type for device %s (%s) with serial %s\n",
				  $chassisinfo->{model},
				  $db_dev->{device_id},
				  $host,
				  $chassisinfo->{serial};
			...
			if ( !( $dev_component_sth->execute(
				$db_dev->{device_id},  $chassisinfo->{manufacturer},
				$chassisinfo->{model}, $chassisinfo->{serial}
			) ) )
			{
				printf STDERR "Unable to insert component: %s\n",
			...
			if ( !$asset->{serial_number} ) {
				if ($verbose) {
					printf
					  "Setting serial number of %s (component id %d) to %s\n",
					  $host,
					  $asset->{component_id},
					  $chassisinfo->{serial};
				}
	...
	if ($encapdomain) {
		...
		$esth->execute( $encapdomain, $encaptype )
		  || die join( ",", $db_dev->{device_id}, $encapdomain, $encaptype ),
		  ": ", $esth->errstr;
		...
		$sth->execute( $db_dev->{device_id}, $encapdomain, $encaptype )
		  || die join( ",", $db_dev->{device_id}, $encapdomain, $encaptype ),
		  ": ", $sth->errstr;
	...
	if ( exists( $chassisinfo->{software} ) && $chassisinfo->{software} ) {

		if ( !( $pull_os_version_sth->execute( $db_dev->{device_id} ) ) ) {
			printf STDERR
			  "Unable to fetch device operating_system version: %s\n",
			...
			if (   !$osrec
				...
				|| $osrec->{version} ne $chassisinfo->{software}->{version} )
			{
				if ($verbose) {
				...
				{
					printf STDERR
					  "Unable to set device operating_system version: %s\n",
