...
sub do_netblock_collection_chooser($) {
	...
	print $cgi->div({-class=>'ncmanip'}, 
		...
		$cgi->div({ -id => 'collec_detail', class => 'irrelevant'},
...
sub build_netblock_collection_add($) {
	...
	return build_collection_row($cgi, undef, "", 
		$cgi->textfield(-name => 'add_NETBLOCK'), "ADD NETWORK");
...
sub build_collection_members($$) {
	...
	my $rv;
	while(my ($id,$ip, $sig, $desc, $devid, $dname) = $sth->fetchrow_array) {
		my $nblink = "";
		my $label = "$ip";
		if ($sig eq 'Y') {
			$label =~ s,/\d+$,/32,;
...
sub process_netblock_collection {
	...
	print $cgi->ul({-class => 'collection'}, 
		$cgi->hidden('NETBLOCK_COLLECTION_ID', $ncid),
		build_collection_row($cgi, undef, undef, "Member", "Description", "RM"),
