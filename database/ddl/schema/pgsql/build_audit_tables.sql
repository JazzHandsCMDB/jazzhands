...
CREATE OR REPLACE FUNCTION build_audit_table(name varchar) 
...
BEGIN
	...
	EXECUTE 'ALTER TABLE audit.' || quote_ident(name ) ||
		...
		ALTER COLUMN "aud#seq" SET DEFAULT nextval('audit.$$ ||
			name || '_seq' || $$')$$;
	PERFORM rebuild_audit_trigger(name);
END;
...
BEGIN
	...
	LOOP
		...
		PERFORM build_audit_table(name);
	END LOOP;
	PERFORM rebuild_audit_triggers();
END;
$FUNC$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION rebuild_audit_trigger(name varchar) RETURNS VOID AS $$
DECLARE
	create_text	VARCHAR;
BEGIN
