...
-- layer3
--
-- another flag to say if it only applies to the default route
-- val table too  [universal and router maybe later endpoint?]
--
-- in the future, this may tie better to services
--
...
-- this probably becomes access control rule
--
-- there's probably some sort of type to specify things that are autogenerated
-- like the vrrp or "all networks behind this one" thing.
--
-- XXX shouldn't be l3 networks, should be netblock collections
--
-- XXX -- this should be a M-M table so rules can be part of mult. groups
-- trigger on rule update that checks for m-m and fails unless variable set.
--
-- should this be an array?  stored procedure that DTRT
--
...
--
-- access_list_assignment table
-- always has a direction - to_network/from_network
-- and one and only one of layer2_network, layer3_network or layer3_interface
-- array of groups - foreign key support via trigger on array ops
--
-- if there is something applied to an interface, then l2/l3 are ignored.
-- we may later want a toggle that says some l2/l3 ones are applied, but then
-- we have the problem of ordering.
--
--
...
--
-- this goes away in favor of an array in the previous table
--
