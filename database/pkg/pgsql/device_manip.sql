...
BEGIN
	...
	SELECT success FROM device_manip.retire_devices(
			device_id_list := ARRAY[ reture_device.Device_id ]
		)
	INTO rv;
...
BEGIN
	...
	--
	PERFORM device_manip.remove_network_interfaces(
...
-------------------------------------------------------------------
CREATE OR REPLACE FUNCTION device_manip.remove_network_interface (
...
BEGIN
	IF network_interface_id IS NULL THEN
		IF device_id IS NULL OR network_interface_name IS NULL THEN
			RAISE 'Must pass either network_interface_id or device_id and network_interface_name to device_manip.delete_network_interface'
	...
	PERFORM * FROM device_manip.remove_network_interfaces(
...
-------------------------------------------------------------------
CREATE OR REPLACE FUNCTION device_manip.remove_network_interfaces (
