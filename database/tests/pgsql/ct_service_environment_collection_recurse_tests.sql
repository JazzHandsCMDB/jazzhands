...
-- \t on
SAVEPOINT ct_service_environment_tests;

\ir ../../ddl/cache/pgsql/create_ct_service_environment_collection_hier_recurse.sql

--
-- Trigger tests
--
CREATE OR REPLACE FUNCTION ct_service_environment_collection_hier_tests() RETURNS BOOLEAN AS $$
DECLARE
	_tally		INTEGER;
	_tal		INTEGER;
	_id		service_environment_collection.service_environment_collection_id%TYPE;
	_members	INTEGER[];
	_colls		INTEGER[];
...
BEGIN
	...
	FOR _i IN 1..8 LOOP
		INSERT INTO service_environment_collection (
			...
			'JHTEST ' || _i, 'JHTEST'
		) RETURNING service_environment_collection_id INTO _id;
		_colls = array_append(_colls, _id);
	END LOOP;

	RAISE NOTICE 'new collections are %', to_json(_colls);

	SELECT COUNT(*) INTO _tal FROM jazzhands_cache.ct_service_environment_collection_hier_recurse;
	RAISE DEBUG '> % records in cache', _tal;

	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_service_environment_collection_hier_recurse',
		new_rel := 'ct_service_environment_collection_hier_recurse',
		prikeys := ARRAY['path']
	);
	...
	WITH ac AS (
		...
		service_environment_collection_id, child_service_environment_collection_id
	) SELECT service_environment_collection_id, _colls[5]
	FROM ac;
	...
	WITH ac AS (
		...
		service_environment_collection_id, child_service_environment_collection_id
	) SELECT _colls[6], service_environment_collection_id
	FROM ac;
	...
	DELETE FROM service_environment_collection_hier
	 	WHERE service_environment_collection_id = _colls[1]
		AND child_service_environment_collection_id = _colls[6]
	;
	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_service_environment_collection_hier_recurse',
	...
	DELETE FROM service_environment_collection_hier
		WHERE service_environment_collection_id = _colls[4]
		AND child_service_environment_collection_id = _colls[6];
	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_service_environment_collection_hier_recurse',
	...
	UPDATE service_environment_collection_hier
		SET service_environment_collection_id = _colls[4]
		WHERE service_environment_collection_id = _colls[3];
	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_service_environment_collection_hier_recurse',
		new_rel := 'ct_service_environment_collection_hier_recurse',
	...
	BEGIN
		...
		UPDATE service_environment_collection_hier
			SET service_environment_collection_id = _colls[7]
			WHERE service_environment_collection_id = _colls[2];
		PERFORM schema_support.relation_diff(
			schema := 'jazzhands_cache',
			old_rel := 'v_service_environment_collection_hier_recurse',
			new_rel := 'ct_service_environment_collection_hier_recurse',
	...
	BEGIN
		...
		DELETE FROM service_environment_collection_hier
			WHERE service_environment_collection_id = _colls[1]
			AND child_service_environment_collection_id = _colls[2];
		PERFORM schema_support.relation_diff(
			schema := 'jazzhands_cache',
			old_rel := 'v_service_environment_collection_hier_recurse',
			new_rel := 'ct_service_environment_collection_hier_recurse',
	...
	BEGIN
		...
		DELETE FROM service_environment_collection_hier
			WHERE service_environment_collection_id = _colls[5]
			AND child_service_environment_collection_id = _colls[6];
		PERFORM schema_support.relation_diff(
			schema := 'jazzhands_cache',
			old_rel := 'v_service_environment_collection_hier_recurse',
...
SELECT ct_service_environment_collection_hier_tests();
-- set search_path=jazzhands;
DROP FUNCTION ct_service_environment_collection_hier_tests();

ROLLBACK TO ct_service_environment_tests;
