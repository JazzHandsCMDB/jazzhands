...
-- \t on
SAVEPOINT ct_layer2_network_tests;

\ir ../../ddl/cache/pgsql/create_ct_layer2_network_collection_hier_recurse.sql

--
-- Trigger tests
--
CREATE OR REPLACE FUNCTION ct_layer2_network_collection_hier_tests() RETURNS BOOLEAN AS $$
DECLARE
	_tally		INTEGER;
	_tal		INTEGER;
	_id		layer2_network_collection.layer2_network_collection_id%TYPE;
	_members	INTEGER[];
	_colls		INTEGER[];
...
BEGIN
	...
	FOR _i IN 1..8 LOOP
		INSERT INTO layer2_network_collection (
			...
			'JHTEST ' || _i, 'JHTEST'
		) RETURNING layer2_network_collection_id INTO _id;
		_colls = array_append(_colls, _id);
	END LOOP;

	RAISE NOTICE 'new collections are %', to_json(_colls);

	SELECT COUNT(*) INTO _tal FROM jazzhands_cache.ct_layer2_network_collection_hier_recurse;
	RAISE DEBUG '> % records in cache', _tal;

	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_layer2_network_collection_hier_recurse',
		new_rel := 'ct_layer2_network_collection_hier_recurse',
		prikeys := ARRAY['path']
	);
	...
	WITH ac AS (
		...
		layer2_network_collection_id, child_layer2_network_collection_id
	) SELECT layer2_network_collection_id, _colls[5]
	FROM ac;
	...
	WITH ac AS (
		...
		layer2_network_collection_id, child_layer2_network_collection_id
	) SELECT _colls[6], layer2_network_collection_id
	FROM ac;
	...
	DELETE FROM layer2_network_collection_hier
	 	WHERE layer2_network_collection_id = _colls[1]
		AND child_layer2_network_collection_id = _colls[6]
	;
	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_layer2_network_collection_hier_recurse',
	...
	DELETE FROM layer2_network_collection_hier
		WHERE layer2_network_collection_id = _colls[4]
		AND child_layer2_network_collection_id = _colls[6];
	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_layer2_network_collection_hier_recurse',
	...
	UPDATE layer2_network_collection_hier
		SET layer2_network_collection_id = _colls[4]
		WHERE layer2_network_collection_id = _colls[3];
	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_layer2_network_collection_hier_recurse',
		new_rel := 'ct_layer2_network_collection_hier_recurse',
	...
	BEGIN
		...
		UPDATE layer2_network_collection_hier
			SET layer2_network_collection_id = _colls[7]
			WHERE layer2_network_collection_id = _colls[2];
		PERFORM schema_support.relation_diff(
			schema := 'jazzhands_cache',
			old_rel := 'v_layer2_network_collection_hier_recurse',
			new_rel := 'ct_layer2_network_collection_hier_recurse',
	...
	BEGIN
		...
		DELETE FROM layer2_network_collection_hier
			WHERE layer2_network_collection_id = _colls[1]
			AND child_layer2_network_collection_id = _colls[2];
		PERFORM schema_support.relation_diff(
			schema := 'jazzhands_cache',
			old_rel := 'v_layer2_network_collection_hier_recurse',
			new_rel := 'ct_layer2_network_collection_hier_recurse',
	...
	BEGIN
		...
		DELETE FROM layer2_network_collection_hier
			WHERE layer2_network_collection_id = _colls[5]
			AND child_layer2_network_collection_id = _colls[6];
		PERFORM schema_support.relation_diff(
			schema := 'jazzhands_cache',
			old_rel := 'v_layer2_network_collection_hier_recurse',
...
SELECT ct_layer2_network_collection_hier_tests();
-- set search_path=jazzhands;
DROP FUNCTION ct_layer2_network_collection_hier_tests();

ROLLBACK TO ct_layer2_network_tests;
