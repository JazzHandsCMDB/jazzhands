...
-- \t on
SAVEPOINT ct_company_tests;

\ir ../../ddl/cache/pgsql/create_ct_company_collection_hier_recurse.sql

--
-- Trigger tests
--
CREATE OR REPLACE FUNCTION ct_company_collection_hier_tests() RETURNS BOOLEAN AS $$
DECLARE
	_tally		INTEGER;
	_tal		INTEGER;
	_id		company_collection.company_collection_id%TYPE;
	_members	INTEGER[];
	_colls		INTEGER[];
...
BEGIN
	...
	FOR _i IN 1..8 LOOP
		INSERT INTO company_collection (
			...
			'JHTEST ' || _i, 'JHTEST'
		) RETURNING company_collection_id INTO _id;
		_colls = array_append(_colls, _id);
	END LOOP;

	RAISE NOTICE 'new collections are %', to_json(_colls);

	SELECT COUNT(*) INTO _tal FROM jazzhands_cache.ct_company_collection_hier_recurse;
	RAISE DEBUG '> % records in cache', _tal;

	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_company_collection_hier_recurse',
		new_rel := 'ct_company_collection_hier_recurse',
		prikeys := ARRAY['path']
	);
	...
	WITH ac AS (
		...
		company_collection_id, child_company_collection_id
	) SELECT company_collection_id, _colls[5]
	FROM ac;
	...
	WITH ac AS (
		...
		company_collection_id, child_company_collection_id
	) SELECT _colls[6], company_collection_id
	FROM ac;
	...
	DELETE FROM company_collection_hier
	 	WHERE company_collection_id = _colls[1]
		AND child_company_collection_id = _colls[6]
	;
	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_company_collection_hier_recurse',
	...
	DELETE FROM company_collection_hier
		WHERE company_collection_id = _colls[4]
		AND child_company_collection_id = _colls[6];
	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_company_collection_hier_recurse',
	...
	UPDATE company_collection_hier
		SET company_collection_id = _colls[4]
		WHERE company_collection_id = _colls[3];
	PERFORM schema_support.relation_diff(
		schema := 'jazzhands_cache',
		old_rel := 'v_company_collection_hier_recurse',
		new_rel := 'ct_company_collection_hier_recurse',
	...
	BEGIN
		...
		UPDATE company_collection_hier
			SET company_collection_id = _colls[7]
			WHERE company_collection_id = _colls[2];
		PERFORM schema_support.relation_diff(
			schema := 'jazzhands_cache',
			old_rel := 'v_company_collection_hier_recurse',
			new_rel := 'ct_company_collection_hier_recurse',
	...
	BEGIN
		...
		DELETE FROM company_collection_hier
			WHERE company_collection_id = _colls[1]
			AND child_company_collection_id = _colls[2];
		PERFORM schema_support.relation_diff(
			schema := 'jazzhands_cache',
			old_rel := 'v_company_collection_hier_recurse',
			new_rel := 'ct_company_collection_hier_recurse',
	...
	BEGIN
		...
		DELETE FROM company_collection_hier
			WHERE company_collection_id = _colls[5]
			AND child_company_collection_id = _colls[6];
		PERFORM schema_support.relation_diff(
			schema := 'jazzhands_cache',
			old_rel := 'v_company_collection_hier_recurse',
...
SELECT ct_company_collection_hier_tests();
-- set search_path=jazzhands;
DROP FUNCTION ct_company_collection_hier_tests();

ROLLBACK TO ct_company_tests;
