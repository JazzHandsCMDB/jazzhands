...
BEGIN
	...
	INSERT INTO device_management_controller (
		device_id, manager_device_id, device_mgmt_control_type, description
	) VALUES (
	...
	IF _r.device_mgmt_control_type != 'testbmc' THEN
		RAISE EXCEPTION 'device_mgmt_control_type is not testbmc: %', _r.device_mgmt_control_type;
	END IF;
	...
	BEGIN
		...
		SET device_id = device3.device_id, device_mgmt_control_type = 'bmctest'
		WHERE device_id = device1.device_id
		AND manager_device_id = device2.device_id
		RETURNING * INTO _r;
		...
		IF _r.device_mgmt_control_type != 'bmctest' THEN
			RAISE EXCEPTION 'device_mgmt_control_type is not bmctest: %', _r.device_mgmt_control_type;
		END IF;
	...
	IF _r.device_mgmt_control_type != 'testbmc' THEN
		RAISE EXCEPTION 'device_mgmt_control_type is not testbmc: %', NEW.device_mgmt_control_type;
	END IF;
